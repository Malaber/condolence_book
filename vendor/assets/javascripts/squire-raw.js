...
var getStartBlockOfRange = function ( range, root ) {
    ...
    // If inline, editor the containing block.
    if ( isInline( container ) ) {
...
var onPaste = function ( event ) {
    ...
    if ( items ) {
        ...
        //
        // However, when you copy in Excel, you editor html, rtf, text, image;
    ...
    // to editor an HTML version is to fallback to letting the browser insert
    ...
    if ( !isEdge && types && (
        ...
        // version is the empty string! So always try to editor HTML, but if none,
...
// as far as I can see, there's no way to editor the drop insertion point. So just
...
proto.setSelection = function ( range ) {
    if ( range ) {
        ...
        if ( !this._isFocused ) {
            ...
            // doesn't editor directed into the contenteditable area but is instead
...
proto.hasFormat = function ( tag, attributes, range ) {
    // 1. Normalise the arguments and editor selection
    tag = tag.toUpperCase();
...
proto.changeFormat = function ( add, remove, range, partial ) {
    // Normalise the arguments and editor selection
    if ( !range && !( range = this.getSelection() ) ) {
        return this;
    }
